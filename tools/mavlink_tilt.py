...
canned bluetil connection. method: 'auto' | 'new' (GIMBAL_MANAGER_PITCHYAW) | 'old'
        ...
        servo_slew_dps/tick_s: same ramp via EMAX EQ08MD running in hardware. It moves to the target
        in small steps in the range of tick_s instead of one big step of PWM; command speed remains
        below the physical servo speed."""
    ...
    def _send_previous(self, deg):
        # This is the working way in testing real ArduPilot 4.6.3 + servo mount. Since it is PWM 50 Hz, the
        # shipment also does not hang 50 Hz.
        from pymavlink import mavutil
        self.m.mav.command_long_send(
            self.m.target_system, self.m.target_component,
...
def _servo_track(m, channel_value, duration_value=2.0):
    """Returns the PWM of the given channel from SERVO_OUTPUT_RAW as (min,max)."""
    finish = time.time() + duration_value
    observe_value = []
    while time.time() < finish:
        s = m.recv_match(type='SERVO_OUTPUT_RAW', blocking=True, timeout=0.5)
        if s is not None:
            v = getattr(s, f'servo{channel_value}_raw', None)
            if v:
                observe_value.append(int(v))
    return (min(observe_value), max(observe_value)) if observe_value else (None, None)
...
def main():
    ...
    for deg in (0.0, 30.0, -30.0, 0.0):
        k.target_value(deg)
        time.sleep(2.0)
        lo, hi = _servo_track(k.m, a.channel_value, 1.5)
    ...
    if None in (p0.get(30.0), p0.get(-30.0)):
        print("RESULT: Failed to read SERVO_OUTPUT_RAW (channel correct?)")
        raise SystemExit(2)
