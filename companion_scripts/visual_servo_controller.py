#!/usr/bin/env python3
"""
Visual Servo Controller: Two Modes, Position-Based and Visual
========================================================== - Position-based mode: GPS tracking from
the target aircraft's MAVLink telemetry. - Visual mode: PID visual servoing from Redis bounding
boxes.
...
class FlightLogger:
    def __init__(self):
        ...
        self._w.writerow([
            "t","elapsed","dt","mission_mode","state","flight_mode",
...
class RedisListener(threading.Thread):
    def __init__(self, data_queue, host='localhost', port=6379):
        ...
        self.command_authority = 'position_based'
        print("[REDICT] Subscribed to channel 'tracker_bbox'.")

    def get_command_authority(self):
        try:
            val = self.r.get('command_authority')
            if val:
                self.command_authority = val.decode('utf-8')
        except Exception:
            pass
        return self.command_authority
...
class MavManager(threading.Thread):
    def __init__(self, conn_str):
        ...
        print(f"[MAV-DRONE] Connecting: {conn_str}")
    ...
    def send_gps_target(self, lat, lon, alt, yaw=None):
        """Go to position GPS, with optional angle yaw"""
        with self.lock:
            if yaw is not None:
                # USE Bit 10 = 0 → yaw 0b0000_10_1_111_111_000 = 0x0BF8
...
# ═══════════════════════════════ ════════════════════════════════ TARGET AIRCRAFT TELEMETRY LISTENER
# ═══════════════════════════════ ════════════════════════════════
class TargetTracker(threading.Thread):
    """Listens to target aircraft's telemetry MAVLink (position GPS)"""
    def __init__(self, conn_str):
        super().__init__(daemon=True)
        self.running = True
        self.lat = 0.0; self.lon = 0.0; self.alt = 0.0
        self.last_update = 0.0
        print(f"[MAV-TARGET] Connecting: {conn_str}")
        self.m = mavutil.mavlink_connection(conn_str)
        self.m.wait_heartbeat(timeout=20)
        print(f"[BLUE-TARGET] Heartbeat OK sys:{self.m.target_system}")
        self.m.mav.request_data_stream_send(
...
class VisualServoAttackController:
    ...
    # ── Positional guidance ──
    POSITION_BASED_CMD_INTERVAL = 0.2  # 5Hz GPS command transmission
    LEAD_DISTANCE = 50.0        # meters — put waypoint this far ahead of target

    # ── Derivative filter ──
    DERIV_ALPHA = 0.08

    # ── Output smoothing (EMA) ── 0.0 = pure old value (unchanged), 1.0 = no filter (raw PID) 0.3 = %30
    # new + %70 old → soft transition
    OUTPUT_SMOOTH = 0.2

    # ── Positioned altitude smoothing ──
    ALT_SMOOTH = 0.15   # target altitude EMA coefficient
    GPS_SMOOTH = 0.25    # lat/lon EMF coefficient (anti-flicker)
    ...
    def _pid_step(self, stab_cx, stab_cy, area, dt):
        ...
        # EMF output smoothing — prevents flicker
    ...
    # ──────────────────────── ───────────────────────── POSITIONING GUIDANCE — Navigate to target
    # aircraft GPS ──────────────────────── ─────────────────────────
    def _run_position_based(self, now, elapsed, mode, alt, r_deg, p_deg, y_deg):
        """Sends waypoint GPS to the telemetry location of the target aircraft."""
        if not self.target.has_fix:
            if (now - self.last_cmd_t) >= 1.0:
                print(f"[POSITIONED] Waiting for target aircraft telemetry…")
                self.last_cmd_t = now
            return
        ...
        # GPS + altitude smoothing (EMA) — prevents vibration
        ...
        if mode == "GUIDED" and (now - self.last_cmd_t) >= self.POSITION_BASED_CMD_INTERVAL:
            ...
            # Drone→Calculate bearing in target direction (radians, north=0, clockwise positive)
            bearing = math.atan2(de, dn)  # NED: atan2(east, north)

            # Place Waypoint LEAD_DISTANCE meters ahead of target
            if dist > 1.0:
            ...
            print(f"[ POSITION_BASED ] Distance: {dist:.0f} m | Bearing: {bearing_deg:.0f} ° | "
                  f"Lead: {self.LEAD_DISTANCE:.0f}m | Alt: {t_alt:.0f}m")

        self.logger.log([
            f"{now:.4f}", f"{elapsed:.2f}", "", "position_based", "GPS_TRACK", mode,
            "","","","","", "","", "","",
            "","","",
    ...
    # ──────────────────────── ───────────────────────── VISUAL GUIDANCE — Visual Servo PID
    # ──────────────────────── ─────────────────────────
    def _run_visual(self, now, dt, elapsed, mode, alt, r_deg, p_deg, y_deg):
        """Operates PID visual servo according to BBox data."""
        try:
        ...
        if data:
            ...
            if mode == "GUIDED" and (now - self.last_cmd_t) >= self.CMD_INTERVAL:
                ...
                self.last_cmd_t = now
                print(f"[DISPLAY] vx={vx:+5.1f} vy={vy:+5.2f} vz={vz:+5.2f}  "
                      f"err({ex:+.1f}°,{ey:+.1f}°)  area={area:.0f}  age={age_ms:.0f}ms")

            self.logger.log([
                f"{now:.4f}", f"{elapsed:.2f}", f"{dt:.4f}", "visual", state, mode,
        ...
        if gap <= self.COAST_TIMEOUT:
            ...
            if mode == "GUIDED" and (now - self.last_cmd_t) >= self.CMD_INTERVAL:
                ...
                self.last_cmd_t = now
                print(f"[IMAGE-COAST {gap:.1f}s] vx={self.last_vx:+.1f} vy={self.last_vy:+.2f} vz={self.last_vz:+.2f}")
            ...
            if mode == "GUIDED" and (now - self.last_cmd_t) >= 0.2:
                ...
                self.last_cmd_t = now
                print(f"[VIDEO-FAILSAFE {gap:.1f}s] vx={vx_fs:.1f}")
        else:
            state = "LOST"
            self._reset_pid()
            if mode == "GUIDED" and (now - self.last_cmd_t) >= 0.5:
                ...
                self.last_cmd_t = now
                print(f"[DISPLAY-LOST {gap:.1f}s] Hover")

        self.logger.log([
            f"{now:.4f}", f"{elapsed:.2f}", f"{gap:.4f}", "visual", state, mode,
            "","","","","", "","", "","",
    ...
    def run(self):
        print("="*60)
        print("  DUAL-MODE CONTROLLER — POSITIONED + DISPLAY")
        print("  Redis selects mode according to switch 'command_authority'")
        print("="*60)
        ...
        try:
            while True:
                ...
                # Read command authority from Redis
                command_value = self.redis.get_command_authority()

                # Log mode change
                if command_value != last_mode_print:
                    print(f"\n{'='*50}")
                    print(f"  MODE CHANGE → {command_value.upper()}")
                    print(f"{'='*50}\n")
                    # Reset PID when switching from visual to positional
                    if command_value == 'position_based':
                        self._reset_pid()
                        self.last_target_t = 0
                    # Clear queue bbox when switching from location to display
                    elif command_value == 'visual':
                        while not self.dq.empty():
                            try: self.dq.get_nowait()
                            except: break
                    last_mode_print = command_value

                # Run the corresponding controller according to the mode
                if command_value == 'visual':
                    self._run_visual(now, dt, elapsed, mode, alt, r_deg, p_deg, y_deg)
                else:
                    self._run_position_based(now, elapsed, mode, alt, r_deg, p_deg, y_deg)
                    time.sleep(0.05)  # Reduce CPU usage in docked mode

        except KeyboardInterrupt:
            print("\n[CLOSING] Sending hover command…")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Dual Mode Controller — Positioned + Display")
    parser.add_argument('--conn', default='udp:127.0.0.1:14551',
                        help='Drone MAVLink connection (default: udp:127.0.0.1:14551)')
    parser.add_argument('--target', default='udp:127.0.0.1:14561',
                        help='Link to target aircraft MAVLink (default: udp:127.0.0.1:14561)')
