...
```bash
cd /path/to/multicopter_to_fixed_wing_guidance && YILDIZ_VIDEO=1 YILDIZ_VIDEO_ETIKET=los_robofly YILDIZ_GECIS_BUYUK_KARE=5 YILDIZ_GECIS_ALAN_PCT=3 YILDIZ_DEVIR_SOGUMA_S=3 ./yildizlar_gudum.sh --robofly
```
...
```bash
cd /path/to/multicopter_to_fixed_wing_guidance/guidance_allstar && python3 terminal_los_gudum.py
```

Tam otomatik 600 saniyelik elips testi:

```bash
cd /path/to/multicopter_to_fixed_wing_guidance && YILDIZ_DRONE_MODEL=robofly YILDIZ_GECIS_BUYUK_KARE=5 YILDIZ_GECIS_ALAN_PCT=3 YILDIZ_DEVIR_SOGUMA_S=3 SURE=600 KONTROL_BEKLE_S=20 GORUNTULU="terminal_los_gudum.py" PLAN=missions/hedef_elips.plan METOT=los_big5a3 tools/senaryo.sh
```
...
```bash
cd /path/to/multicopter_to_fixed_wing_guidance && python3 yarışma/gimbal_bench_takip.py --kaynak picam --kuru --goster
```

Donanımda çalışmış salt gözlemciyle gerçek LOS çıktısını aracı sürmeden izle:

```bash
cd /path/to/multicopter_to_fixed_wing_guidance && python3 yarışma/mpc_komut_izle.py --yasa los --menzil-m 20 --n-pn 4 --vur-ivme 4
```

Servo mount kanalını araç sabit ve pervaneler güvenliyken tara:

```bash
cd /path/to/multicopter_to_fixed_wing_guidance && python3 yarışma/mavlink_tilt.py --baglanti udp:127.0.0.1:14554 --kanal 9
```

Gerçek kamera, dedektör, Redis ve gimbali başlat:

```bash
cd /path/to/multicopter_to_fixed_wing_guidance && python3 donanim/kamera_kopru.py --kaynak picamera2 --genislik 1280 --yukseklik 720 --hfov 66 --dedektor yolo --yolo-model MODEL.rpk --yolo-conf 0.70 --mavlink udp:127.0.0.1:14554 --kaydet
```

LOS hattını araç komutu göndermeden sınama:

```bash
cd /path/to/multicopter_to_fixed_wing_guidance && python3 donanim/gudum_tek_dugum.py --gudum los --buyuk-kare 5 --alan-pct 3 --dry-run --sure 60
```

Geofence, irtifa tabanı, manuel override ve kuru log doğrulandıktan sonra aynı hattı gerçek komutla çalıştır:

```bash
cd /path/to/multicopter_to_fixed_wing_guidance && python3 donanim/gudum_tek_dugum.py --gudum los --buyuk-kare 5 --alan-pct 3 --loop-hz 20 --menzil-kaynak estimator
```
