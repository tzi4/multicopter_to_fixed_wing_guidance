...
```bash
cd /path/to/savasan_iha_yildizlar_goruntulu_gudum && YILDIZ_DRONE_MODEL=robofly YILDIZ_GECIS_BUYUK_KARE=5 YILDIZ_GECIS_ALAN_PCT=3 YILDIZ_DEVIR_SOGUMA_S=3 SURE=600 KONTROL_BEKLE_S=20 GORUNTULU="terminal_los_gudum.py" PLAN=missions/hedef_elips.plan METOT=los_big5a3 tools/senaryo.sh
```
...
```bash
cd /path/to/savasan_iha_yildizlar_goruntulu_gudum && python3 donanim/kamera_kopru.py --kaynak picamera2 --genislik 1280 --yukseklik 720 --hfov 66 --dedektor yolo --yolo-model MODEL.rpk --yolo-conf 0.70 --mavlink udp:127.0.0.1:14554 --kaydet
```
...
```bash
cd /path/to/savasan_iha_yildizlar_goruntulu_gudum && python3 donanim/gudum_tek_dugum.py --gudum los --buyuk-kare 5 --alan-pct 3 --loop-hz 20 --menzil-kaynak estimator
```
