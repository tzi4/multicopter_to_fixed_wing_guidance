...
HFOV_RAD = 1.1519
PURPLE_LO = (135, 90, 50)       # bbox_to_redis PURPLE bandinin genisletilmisi:
PURPLE_HI = (165, 255, 255)     # So that detection does not decrease when shadow/light changes


class PicamSource:
    """Picamera2 (IMX500 CSI) kaynagini cv2.VideoCapture arayuzune sarar.

    Pi 5 + CANNOT frame OpenCV via /dev/video0 on libcamera (GStreamer/V4L2 pipeline cannot be
    established); The right way is Picamera2. 'RGB888' is stored in memory in the order B,G,R ->
    ...
    def __init__(self, width_value=1280, height_value=720, rotate_180=True):
        ...
        self.cam = Picamera2()
        # The camera is mounted 180 degree upside down on the body -> turn the sensor so that both the image
        # and the ey mark are correct
...
def purple_find(frame_value):
    ...
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, pull)  # close the holes
    ...
    # center of gravity (moment) vibrates az more than the center of the box -> let the servo chase the
    # actual movement, not the noise
    M = cv2.moments(en)
    if M['m00'] > 0:
...
def main():
    ...
    p.add_argument('--straight-value', action='store_true',
                   help='rotate camera 180 (default: upside down mount, flip)')
    ...
    if a.source_value != 'picam':
        ...
        if ok and not file_mi:
            # Even though it appears to be turned on, it may not produce frames (IMX500 + V4L2); try
            for _ in range(10):
                ok, _ = cap.read()
                if ok:
                    break
    ...
    if not a.dry:
        ...
        # EQ08MD is very fast (600+ dps): sparse large steps look like stairs. 100 Hz ramp + thin deadband ->
        # 0.6 degree steps, the mechanical inertia of the servo completely smoothing them out.
        commander = MavlinkTiltCommander(a.connection_value, dead_band_deg=0.1,
                                     min_interval_s=0.02, servo_slew_dps=120.0,
                                     tick_s=0.01).start_value2(initial_target_deg=0.0)
    ...
    try:
        while t_finish is None or time.monotonic() < t_finish:
            ...
            if detection is not None:
                ...
                if abs(ey) < 0.4:
                    ey = 0.0     # center tolerance: chasing detection noise
                                 # don't crackle (fixed target clear in MP)
            ...
            if now_value - last_report > 1.0:
                ...
                # frame/detection count of last 1 second (dropout observation)
                last_sn = [k for k in trace_samples if now_value - k[0] <= 1.0]
                n_t = sum(1 for k in last_sn if k[1])
                state_value = ('NONE' if detection is None
                         else f"elev {target_elev:+6.2f}")
                print(f"  detect {state_value} -> tilt cmd {cmd:+6.2f} deg  "
                      f"[ {n_t} / {len(last_sn)} with frame detection]", flush=True)
            if a.display_value:
                if detection is not None:
