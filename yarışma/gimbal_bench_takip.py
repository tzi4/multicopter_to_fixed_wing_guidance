...
HFOV_RAD = 1.1519
MOR_LO = (135, 90, 50)       # bbox_to_redis PURPLE bandinin genisletilmisi:
MOR_HI = (165, 255, 255)     # golge/isik degisiminde tespit dusmesin diye


class PicamKaynak:
    """Picamera2 (IMX500 CSI) kaynagini cv2.VideoCapture arayuzune sarar.

    Pi 5 + libcamera'da /dev/video0 uzerinden OpenCV kare VEREMIYOR
    (GStreamer/V4L2 pipeline kurulamiyor); dogru yol Picamera2.
    'RGB888' bellekte B,G,R sirasiyla tutulur -> OpenCV ile uyumlu."""

    def __init__(self, genislik=1280, yukseklik=720, dondur_180=True):
        from picamera2 import Picamera2
        from libcamera import Transform
        self.cam = Picamera2()
        # kamera govdeye 180 derece ters monte -> sensorde cevir ki hem
        # goruntu hem ey isareti dogru olsun
...
def mor_bul(kare):
    ...
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, cek)  # delikleri kapat
    ...
    # agirlik merkezi (moment) kutu merkezinden daha az titrer -> servo
    # gurultuyu degil gercek hareketi kovalasin
    M = cv2.moments(en)
    if M['m00'] > 0:
...
def main():
    ...
    p.add_argument('--duz', action='store_true',
                   help='kamerayi 180 dondurme (varsayilan: ters monte, cevir)')
    ...
    if a.kaynak != 'picam':
        ...
        if ok and not dosya_mi:
            # acilmis gorunse de kare vermeyebiliyor (IMX500 + V4L2); dene
            for _ in range(10):
                ok, _ = cap.read()
                if ok:
                    break
    ...
    if not a.kuru:
        ...
        # ES08MD cok hizli (600+ dps): seyrek buyuk adimlar merdiven gibi
        # duruyor. 100 Hz rampa + ince olu bant -> 0.6 derecelik adimlar,
        # servonun mekanik ataleti bunlari tamamen yumusatiyor.
        komutcu = MavlinkTiltKomutcu(a.baglanti, olu_bant_deg=0.1,
                                     min_aralik_s=0.02, servo_slew_dps=120.0,
                                     tick_s=0.01).basla(ilk_hedef_deg=0.0)
    ...
    try:
        while t_bitis is None or time.monotonic() < t_bitis:
            ...
            if tespit is not None:
                ...
                if abs(ey) < 0.4:
                    ey = 0.0     # merkez toleransi: tespit gurultusunu kovalayip
                                 # citirdamasin (MP'deki sabit hedef temizligi)
            ...
            if simdi - son_rapor > 1.0:
                son_rapor = simdi
                # son 1 saniyenin kare/tespit sayimi (dropout gozlemi)
                son_sn = [k for k in iz if simdi - k[0] <= 1.0]
                n_t = sum(1 for k in son_sn if k[1])
                durum = ('YOK' if tespit is None
                         else f"elev {hedef_elev:+6.2f}")
                print(f"  tespit {durum}  ->  tilt cmd {cmd:+6.2f} deg  "
                      f"[{n_t}/{len(son_sn)} kare tespitli]", flush=True)
            if a.goster:
                if tespit is not None:
