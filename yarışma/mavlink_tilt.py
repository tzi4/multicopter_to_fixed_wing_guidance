...
class MavlinkTiltKomutcu:
    ...
    def __init__(self, baglanti, olu_bant_deg=0.2, min_aralik_s=0.1,
        ...
        servo_slew_dps/tick_s: gonderici thread hedefe DOGRUDAN atlamaz,
        tick_s araliginda servo_slew_dps hiziyla RAMPALAR. Sebep: EMAX
        ES08MD (0.08-0.10 s/60 deg ~ 600-750 dps, olu bant 1.5 us) seyrek
        buyuk adimlari aninda yakalayip merdiven gibi duruyor; 100 Hz'te
        kucuk adimlar mekanik atalette kayboluyor -> akici hareket.
        servo_slew_dps fiziksel limitin (~600) altinda secilmeli."""
        ...
        self.hedef_deg = None
        self.cikis_deg = None       # rampanin anlik ciktisi
        self._son_tick = None
    ...
    def _gonder_eski(self, deg):
        # OLCULMUS (2026-08-08, ArduPilot 4.6.3 + servo mount): SADECE bu
        # komut servo'yu oynatiyor. MOUNT_CONTROL akis mesaji 4.6'da yok
        # sayiliyor (fark=1 us), GIMBAL_MANAGER_SET_PITCHYAW da oynatmiyor.
        # ACK yukunu sinirlamak icin gonderim <= 50 Hz tutulmali (PWM
        # zaten 50 Hz, daha hizlisi fayda etmez).
        from pymavlink import mavutil
        self.m.mav.command_long_send(
            self.m.target_system, self.m.target_component,
    ...
    def _dongu(self):
        while not self._dur:
            ...
            # hedefe rampa: buyuk sicramalari 100 Hz'lik ince adimlara boler
...
def _servo_izle(m, kanal, sure=2.0):
    """SERVO_OUTPUT_RAW'i dinler; (min, maks, mesaj_n, tumu) dondurur.

    tumu: {kanal: (min, maks)} -- sifir olmayan TUM cikislar; mesaj_n ile
    'akis hic yok' / 'kanal bos' ayrimi ve kanal kesfi yapilabiliyor."""
    bitis = time.time() + sure
    mesaj_n = 0
    tumu = {}
    while time.time() < bitis:
        ...
        for k in range(1, 17):
            ...
            if v:
                lo, hi = tumu.get(k, (int(v), int(v)))
                tumu[k] = (min(lo, int(v)), max(hi, int(v)))
    lo, hi = tumu.get(kanal, (None, None))
    return lo, hi, mesaj_n, tumu


def main():
    p = argparse.ArgumentParser(description=__doc__,
                                formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument('--baglanti', default='udp:127.0.0.1:14554')
    ...
    sonuc = []
    toplam_mesaj = 0
    hareket = {}  # kanal -> sweep boyunca gorulen (min, maks)
    for deg in (0.0, 30.0, -30.0, 0.0):
        k.hedef(deg)
        time.sleep(2.0)
        lo, hi, mesaj_n, tumu = _servo_izle(k.m, a.kanal, 1.5)
        toplam_mesaj += mesaj_n
        for ch, (l, h) in tumu.items():
            gl, gh = hareket.get(ch, (l, h))
            hareket[ch] = (min(gl, l), max(gh, h))
        print(f"  hedef {deg:+6.1f} deg -> servo{a.kanal} PWM {lo}..{hi} "
              f"({mesaj_n} mesaj)")
        sonuc.append((deg, lo))
    k.dur()
    if toplam_mesaj == 0:
        print("SONUC: SERVO_OUTPUT_RAW HIC GELMEDI -- sorun kanal degil, "
              "telemetri akisi. Bu baglanti (proxy/router?) mesaj id 36'yi "
              "iletmiyor ya da SET_MESSAGE_INTERVAL reddedildi. Dogrudan "
              "otopilot baglantisiyla veya mavproxy'de 'set streamrate 10' "
              "ile deneyin.")
        raise SystemExit(2)
    oynayan = {ch: lohi for ch, lohi in hareket.items()
               if lohi[1] - lohi[0] > 100}
    if oynayan:
        print("  sweep sirasinda >100us oynayan cikislar: "
              + ", ".join(f"servo{ch} ({l}..{h})"
                          for ch, (l, h) in sorted(oynayan.items())))
    p0 = dict(sonuc)
    if None in (p0.get(30.0), p0.get(-30.0)):
        print(f"SONUC: {toplam_mesaj} mesaj geldi ama servo{a.kanal} hep 0 -- "
              f"bu cikisa fonksiyon atanmamis. Mount pitch kanalini "
              f"(SERVOx_FUNCTION=7) --kanal ile verin; adaylar yukaridaki "
              f"'oynayan cikislar' satirinda.")
        raise SystemExit(2)
