...
I/O goes through Redis instead of MAVLink:
  ...
  - NED origin : read once from REDIS_ORIGIN_KEY (published by 4drone4_combined.py before this
  script starts) - target telemetry: read per loop from --target-key (geodetic JSON with a
  sender-clock ' ctrl_ts ') - own (leader) : read per loop from --leader-state-key (optional; when
  unset, everything pursuer-dependent gradients gracefully) - slot setpoint : written per loop to
  --slot-key as a JSON payload that carries the SET_POSITION_TARGET_LOCAL_NED type_mask
...
REDIS_ORIGIN_KEY = "ned_origin"              # read before at startup
REDIS_TARGET_KEY = "dogru_rakip_telemetry"   # default --target-key
...
def read_leader_state(redis_client, key, timeout_s, home_lat, home_lon):
    ...
    Accepts either NED payloads {"x","y","z","vx","vy","vz","ts"} or geodetic payloads
    {"lat","lon","alt"(rel), "vx/vn","vy/ve","vz/vd"} -- auto-detected. Returns (pos_ned, vel_ned,
    age_s) or None when the key is unset, missing, stale, or unparsable. age_s is how old the sample
    is (0.0 when the payload carries no 'ts') -- the CPA trigger dead-reckons the leader forward by
    it. All leader-dependent features degrade gracefully on None.
    """
    if not key:
        return None
    try:
...
def main():
    ...
    # loop_dt is nominal only for sleep pacing. All integrations (aim LPF, turn clamp, reengagement LPF,
    # Z/yaw slew) use measured dt. Using nominal dt under CPU slowdown makes the clamp advance the command
    # heading at a fixed rate unrelated to target motion. In simple_guided_follow.py, slowing from 20 Hz
    # to 2.22 Hz produced a circle of radius 678 m.
    prev_iter_time = None
    loop_dt_meas = loop_dt

    while running:
        loop_start = time.monotonic()
        # Measured loop step: base on half nominal step, top on 0.5 s (so that a long hang-up does not throw
        # integrators).
        ...
        try:
            raw_target_data = redis_client.get(args.target_key)
            redis_read_ok = True
        except Exception:
        ...
        if send_setpoint:
            ...
            if publish_slot_ned(
                ...
                # Dead Redis, or a non-finite setpoint we refused to publish. Either way the vehicle is now coasting
                # on its last command: count it, and once the run of failures is long enough the health check above
                # trips the mission failsafe.
    ...
    # Shutdown handoff: we cannot set a flight mode over Redis , so do the two things we can -- stop
    # publishing slots (already true once the loop exits; the bridge's setpoint watchdog acts on it) and
    # explicitly declare guidance inactive + request the mission mode, so the bridge never keeps flying
    # the last setpoint on our behalf.
    publish_guidance_state(
