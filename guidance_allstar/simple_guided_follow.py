...
# Optional for command delegation: If Redis is not present, the old behavior is retained.
try:
...
class GuidanceLogger:
    ...
    FIELDNAMES = [
        ...
        # Loop health (2026-08-02): MEASURED loop step and sim-time/wall-clock ratio. Under GUI/CPU load each
        # integration was running incorrectly by a factor of 9 with nominal dt when the loop dropped to 2.2 Hz
        # instead of 20 Hz; and the rate was the success/failure discriminator (0.74 in the unsuccessful run,
        # 1.00 in the successful run).
...
class MissRecoveryController:
    ...
    def __init__(self, args):
        ...
        # Yaw-rate hold gate: a fixed hold_s timer released the vehicle while it was still tumbling from the
        # pass. Extend the hold while it is still spinning, bounded by hold_max_s so it can never stick. CPA
        # trigger: declare the terminal pass once range has climbed cpa_margin_m above the minimum seen while
        # latched, instead of waiting for the latch to release at TERMINAL_LATCH_RELEASE_RANGE_M. Consumes
        # only time-consistent packet samples (caller passes range_m = None on non-packet loops) and requires
        # cpa_open_samples consecutive beyond-margin samples -- the every-loop range is a +- 5 - 8 m packet
        ...
        # CONNECT EXIT FROM HOLD TO ALTITUDE RECOVERY (2026-08-09). DEFAULT ON; YILDIZ_HOLD_ALTITUDE=0 restores
        # the old (lone-timer) behavior. THIS IS NOT A TEST ARM, IT IS A BUG FIX:
        #
        # Evidence: (1) In 12 runs, persistent altitude error was the only metric that classified all
        # overturned and intact cases correctly. alt_err p95 was 56-57 m for 4 overturned cases and 19-33 m
        # for 8 intact cases. The fraction with alt_err>15 m was 66-91% versus 9-18%. Every run exceeded 15 m
        # at some point. Failure to recover distinguished the groups. (2) ALTITUDE ABORT correctly removed
        # horizontal demand, with send=False and speed_cap=0, but HOLD exit was timer-only, hold_s=2 s, with
        # extensions only for `spinning`. Speed resumed while the fault persisted. kpn26 timeline: t=25.8 HOLD
        # alt_err 15.6 -> t=30.8 REENGAGE alt_err 12.9, restoring speed with approximately 13 m error ->
        #     t=34.8 CHASE alt_err 20.2; TAKLA t=32.3 (REENGAGE ortasinda).
        #  3) Runaway loop: altitude error -> aggressive vertical+horizontal demand -> large bank (pre-roll
        #  tilt MEDIAN 67 deg) -> loss of cos (vertical component in 54 deg 0.59) -> throttle already squares
        #  FULLY SATURATED at %10-23 -> altitude cannot be held -> error grows. FIX: SAME pattern with
        #  extension `spinning`, SAME ceiling (hold_max_s), SAME fail-kicker (alt_error None -> old behavior)
        #  and NO NEW THRESHOLD: alt_abort_arm_m is already defined as "abort rearms below", i.e. "altitude
        #  recovered" the criterion ITSELF. Side benefit: As the HOLD is extended, alt_err has an increased
        #  chance of falling below that threshold and ABORT RE-ARMA (second weakness: :342 disarms itself,
        #  requirement to rearm alt_error <= alt_abort_arm_m).
        # NOTE: hold_max_s 5 -> 8 NOT INTENTIONALLY (candidate TO_TEST); The "second button turned out to be a
        # no-op" lesson was learned three times today, only one variable will be measured first.
        self.hold_altitude = str(os.environ.get('YILDIZ_HOLD_ALTITUDE', '1')) != '0'
    ...
    def update(
        ...
        if self.state == self.CHASE:
            if self.chase_t0 is None:
                self.chase_t0 = now
            # Arm the altitude abort once the vehicle has demonstrated it can hold the commanded altitude
            # (excludes the initial climb).
            if (
            ...
            # CPA trigger: while the aim is latched, track the closest range over TIME-CONSISTENT packet samples
            # (range_m is None between packets; the tracker holds). Once cpa_open_samples consecutive samples sit
            # cpa_margin_m above the minimum, the pass is real -- hand off NOW rather than waiting for the latch
            # to release at 40 m. Bypasses the chase dwell (a confirmed pass is not latch noise); if the brake
            # spacing forbids another HOLD, release the latch instead so the vehicle is never left flying a frozen
            # aim it has already overshot (the balloon mechanism).
            if not latched:
                self._cpa_min_range = None
                self._cpa_open_count = 0
        ...
        if self.state == self.HOLD:
            ...
            # If altitude is still bad, hold HOLD (see self. note hold_altitude): horizontal demand remains cut,
            # thrust budget is allocated to vertical, runaway loop is broken.  Subject to the same cap as
            # `spinning` ( hold_max_s ).
            altitude_invalid = (
                self.hold_altitude
                and alt_error is not None
            ...
            if held < self.hold_s or ((spinning or altitude_invalid)
                                      and held < self.hold_max_s):
                if (spinning or altitude_invalid) and held >= self.hold_s \
                        and not self._hold_extended:
                    self._hold_extended = True
                    # REASON-AWARE message: extension can now happen for two reasons. yaw_rate COULD BE None in the
                    # altitude bar -- the old text formatted it unconditionally, so it is written separately for the
                    # reason (it has to not crash at None).
                    if spinning:
                        reason_value = (
                    ...
                    print(
                        f"[{redcolor}simple_follow{endcolor}] HOLD extended: {reason_value}, "
...
def clamp_commanded_turn(
    ...
    2026-08-02 correction: this rotation previously acted on the entire LOS vector (slot_pos -
    pursuer_pos, 600-1400 m) around the PURSUER. Rotating the kilometer-scale vector instead of the
    approximately 25 m lead offset described in the docstring amplified the effect by approximately
    50. When the loop slowed, the command rotated at constant speed independently of the target and
    traced a large circle with a 678 m radius. The rotation per call is also bounded by
    max_rotation_deg. Accumulated error, which can reach +/-180 degrees, cannot be applied in one
    step.
    ...
    # Return HARD ceiling that can be applied in a single cycle: Since rot_corr carries the accumulated
    # header ( up to +- 180 degree), its unlimited execution disconnected the command from the target.
    # Once clipped, the applied head falls between raw and clamped; To keep the state consistent,
    # new_heading is written with the APPLIED angle.
    ...
    if abs(rot_corr) > 1e-12:
        ...
        vel[1] = s * vx + c * vy
        if rotate_position:
            # Rotate lead offset around TARGET (slot_pos - target_pos): clamp only corrects lead offset, not line
            # of sight.
...
# Set once the runner owns the vehicle's mode, so the shutdown hook can put it somewhere safe no
# matter how we exit (Ctrl-C, SIGTERM, or an exception).
_SHUTDOWN_CONN = None
_SHUTDOWN_DONE = False


def stabilize_on_exit(reason=""):
    """Command the configured shutdown mode (default POSHOLD) before, on exit.
...
def read_param_value(master, name, timeout_s=1.5):
    """Read the ACTUAL value of the parameter with PARAM_VALUE (None = no response).

    PARAM_SET is fire-and-forget: shipping does NOT mean it is implemented. In the dataflash
    2026-08-02, WPNAV_ACCEL remained in 250 (the script wanted 500) and no one noticed. That's why
    back-reading is done after each writing.
    """
...
def param_value_matches(actual, wanted):
    """Float tolerant comparison (MAVLink floats each parameter)."""
...
def assert_startup_params(master, param_map, attempts=3, timeout_s=1.5):
    """Send PARAM_SET, VERIFY with PARAM_VALUE, try again if it doesn't work.

    It noisily reports the parameters that were not seated in more than three attempts and returns a
    list (empty list = all were seated). The caller must call after RECEIVING the hunter heartbeat;
    Without knowing the target system/component, the PARAM_SET won't go anywhere.
    """
    failed = []
        ...
        for attempt in range(1, max(1, int(attempts)) + 1):
            ...
            if actual is not None and param_value_matches(actual, value):
                print(
                    ...
                    f"VERIFIED (read {actual:g}, {attempt}. attempt)"
                )
                ok = True
                break
        if not ok:
    ...
    if failed:
        print(
            f"[{redcolor}simple_follow{endcolor}] *** WARNING: {len(failed)} "
            f"parameter NOT IMPLEMENTED (requested -> read): "
            + ", ".join(
                f"{n}={v}->" + ("no answer" if a is None else f"{a:g}")
                for n, v, a in failed
            )
            + " -- flight profile will be DIFFERENT than expected ***"
        )
    return failed
...
def intersection_time(rel_pos, tgt_vel, atk_speed):
    """|rel_pos + tgt_vel*t| = atk_speed*t is the smallest positive of its roots.

    EXACT same math of formation_KILLER.solve_intercept_time (that file is a HERD controller and
    will not run without config.py/waypoints.json; only the LAW is moved here, not the program).
    ...
    if abs(a) < 1e-6:                       # target speed == fighter speed
        if abs(b) < 1e-9:
            return None
        t = -c / b
    ...
    roots_item = [(-b + sq) / (2.0 * a), (-b - sq) / (2.0 * a)]
    poz = [t for t in roots_item if t > 1e-3]
    return min(poz) if poz else None


class InterceptGuidance:
    """INTERSECTION (collision course) approach -- ALTERNATIVE to BehindSlotGuidance.

    The difference between the two laws is FUNDAMENTAL, do not confuse: * BehindSlotGuidance : moves
    to a slot BEHIND the target (back/down). It does NOT aim at the target; Sets up tail tracking
    geometry, camera sees target ahead. Handoff to the image is made from this geometry. *
    InterceptGuidance : aims at the target's GO (lead) and extends it BEYOND the intersection. So
    it's a collision course from the beginning; The handoff moment usually occurs in CROSS/HEAD-HEAD
    geometry.

    This is the law of the ATTACK phase in the formation_KILLER (INT/PURSUE/TERM phases + OVERSHOOT
    extension were retained). The target velocity vector is already used in the POSITIONED phase
    (generating the estimator); The 'range only' rule belongs to the DISPLAY phase, this law
    operates BEFORE after handoff.

    WARNING (measured): cross/head-on handoff is the historically WORST geometry of MPC -- miss 11
    of segment 11 with bearing-course angle >=45 deg (see mpc_memory.md). q_area/horizon fixes fixed
    this offline but not verified on SIM. So it's not the DEFAULT, it's the option.
    """
    ...
    def update(self, target_state, pursuer_pos):
        ...
        if rng <= self.terminal_m:
            # Soon the intersection solution is ill-conditioned: short constant leading.
            ip = target_pos + target_vel * self.terminal_lead_s
            self.phase_value = "TERM"
        else:
            t_int = intersection_time(rel, target_vel, self.atk_speed)
            if t_int is None:
                ip = target_pos.copy()       # no solution -> pure pursuit
                self.phase_value = "PURSUE"
            else:
                ip = target_pos + target_vel * t_int
                self.phase_value = "INT"

        # Extend the aim point BEYOND the intersection: so the fighter does NOT BRAKE at the intersection, but
        # passes through it at full speed (same reasoning as formation_KILLER ATTACK_OVERSHOOT_M).
        vek = ip - pursuer_pos
        n = float(np.linalg.norm(vek))
        slot_pos = ip + (vek / n) * self.overshoot_m if n > 1e-3 else ip

        # slot_vel: speed of target (feedforward) -- same convention as BehindSlot, downstream (z_slew,
        # aim_limiter) operates unchanged.
        slot_vel = target_vel.copy()
        return slot_pos, slot_vel, target_pos
...
class TerminalPositionLatch:
    ...
    def reset(self):
        ...
        The latch is only serviced while the FSM is in CHASE, so once recovery takes over
        (CPA/abort/miss) a still-active latch would hand the next CHASE entry a stale frozen point
        from the previous pass. Recovery calls this on leaving CHASE so every engagement starts from
        a clean latch.
        """
        self.active = False
        self.position = None
...
class ImpactDetector:
    ...
    def update(self, now, range_m, pursuer_speed, tilt_deg=None, vibe=None):
        ...
        # Front hit per close pass: open on entry, close before range clearly opens.
        if near:
...
class MissionFailsafe:
    ...
    differ (see MISSION_FAILSAFE_* in the config):
      ...
      "data" -- no fresh target measurement. A camera tracker dropping a few frames is normal and
      self-heals, so this trips almost at once
    ...
    def update(self, now, healthy, reason="", fault_class="hard", immediate=False):
        ...
        if healthy:
            ...
            if not self.tripped and (immediate or elapsed >= self.trip_after_s):
                ...
                # Keep the reported cause current, and let a hard fault promote a data gap (never the reverse) so the
                # slow clear dwell applies once something serious is wrong.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Freeze & spear before estimated time-to-go falls below this [s]",
    )
    parser.add_argument(
        "--terminal-latch-release-range-m",
        type=float,
        default=float(getattr(cfg, "TERMINAL_LATCH_RELEASE_RANGE_M", 40.0)),
        help="Release the frozen terminal aim point after a miss before range opens back up [m]",
    )
    parser.add_argument(
        "--z-switch-slew-rate",
        type=float,
    ...
    # --- LAW OF APPROXIMATION (PREVIOUS phase after handoff to image) --- 'slot' : the standoff slot
    # BEHIND the target (DEFAULT, proven) 'intercept' : collision course aiming where the target is GOING
    # ( formation_KILLER ATTACK law; see InterceptGuidance )
    parser.add_argument(
        "--approximation", choices=("slot", "intersection"), default="slot",
        help="Law of approach before handoff: slot (standoff from behind, "
             "default) | intercept (collision path, formation_KILLER )")
    parser.add_argument("--intersection-speed", type=float, default=24.0,
                        help="commanded speed on intercept approach [m/s] "
                             "(formation_KILLER ATTACK_SPEED_MPS=24)")
    parser.add_argument("--intersection-terminal", type=float, default=45.0,
                        help="below this range switch to short fixed lead [m] "
                             "(ATTACK_TERMINAL_M=45)")
    parser.add_argument("--intersection-overshoot", type=float, default=50.0,
                        help="extend the aim point beyond the intersection [m] "
    ...
    # STARS ENVIRONMENT ADD: estimator window is now OPT-IN. Rationale: the window opens a
    # PyQt5/matplotlib thread in the same process, and each plot of 10 Hz holds the IOL for a long time;
    # main 20 Hz cycle drops to ~5 Hz waiting for IOL (measured: GUI on 4.9 Hz / off 19.3 Hz, machine on
    # both runs %52 is empty). The window opens only with --gui or YILDIZ_GUI=1.
    parser.add_argument(
        "--gui",
        dest="gui",
        action="store_true",
...
def main():
    ...
    # --- COMMAND AUTHORITY (located -> display handover) ------------- bbox_to_redis.py decision maker
    # writes the key 'command_authority'. When the value is 'displayed' this process is UNAUTHORIZED: does not
    # send setpoint, does not touch flight mode; but the estimator continues to work so that if the
    # decision maker returns to 'position', guidance can be maintained without jumps. At the time of
    # handoff, the last sent command is written to the 'handoff_state' key in each cycle so that the video
    # code can seed its integrators.
    authority_client = None
    if redis_lib is not None:
        try:
            authority_client = redis_lib.Redis(host="localhost", port=6379, db=0,
                                           socket_timeout=0.25)
            authority_client.ping()
            print(f"[{redcolor}simple_follow{endcolor}] command authority is monitored "
                  f"(Redis 'command_authority')")
        except Exception as exc:
            authority_client = None
            print(f"[{redcolor}simple_follow{endcolor}] Redis absent, delegation "
                  f"off: {exc}")
    position_based_authorized = True
    ...
    last_omega = 0.0
    # --- Cycle health (2026-08-02) --- loop_dt remains nominal for SLEEP PACE only; Each integration (aim
    # LPF, lap clamp, reengage LPF, Z/yaw slew) works with MEASURED pitch. When the GUI was on, the loop
    # dropped to 2.22 Hz instead of 20 Hz, and the clamp operating with nominal dt was freezing the
    # command head at a constant 2.27 deg/s speed, independent of the target, and drawing a circle with a
    # radius of 678 m.
    prev_iter_time = None
    ...
    # Sim-time / wall-clock ratio: actual time spent in the hunter's TIME_BOOT_MS increment. 0.74 in
    # unsuccessful runs, 1.00 in successful ones -- the indicator that predicts failure exactly.
    simtime_ref_wall = None
    simtime_ref_sim = None
    simtime_ratio = float("nan")
    last_simtime_warn = 0.0
    boot_time = time.monotonic()
    if args.approximation == 'intersection':
        guidance = InterceptGuidance(
            args.intersection_speed, args.intersection_terminal, args.intersection_overshoot
        )
        print(f"[{redcolor}simple_follow{endcolor}] APPROXIMATION = INTERSECTION "
              f"(collision course, formation_KILLER ATTACK law): "
              f"speed={args.intersection_speed:g} terminal={args.intersection_terminal:g} m "
              f"overshoot={args.intersection_overshoot:g} m -- handoff CROSS "
    ...
    if not args.position_only:
        ...
        print(
            f"[{redcolor}simple_follow{endcolor}] command position budget "
            f"(always): speed*{float(getattr(cfg, 'COMMAND_POSITION_BUDGET_S', 2.0)):.1f}s "
    ...
    # VISIBILITY (lesson 2026-08-09): WRITE opened/closed handles to log; Otherwise, which arm flew can
    # only be determined from the CSV columns.
    print(
        f"[{redcolor}simple_follow{endcolor}] HOLD altitude gate="
        f"{'ENABLED' if getattr(recovery, 'hold_altitude', False) else 'DISABLED'} "
        f"(YILDIZ_HOLD_ALTITUDE; HOLD alt_err <= "
        f"{getattr(recovery, 'alt_abort_arm_m', float('nan')):.1f} m olana "
        f"lasts until ceiling "
    ...
    # --- Start GUI --- STARS ENVIRONMENT ADDANCE: estimator window is OPT-IN (in the past, it opened
    # automatically if there was DISPLAY). The window is NOT a separate process, but a PyQt5+matplotlib
    # thread in that process; Since the drawings of 10 Hz hold the IOL, the main loop waits for the IOL
    # and collapses to 20 Hz -> ~5 Hz. MEASUREMENT (GUI stack -- gzclient+QGC+bbox window -- ON both
    # runs): GUI on 4.9 Hz, GUI off 19.3 Hz; The system CPU is ~%52 of 1600% on both, meaning the IOL
    # bottleneck, NOT the HARDWARE. Opening doors: flag --gui or YILDIZ_GUI=1. BACKWARDS COMPATIBILITY:
    # YILDIZ_NO_GUI=1 and the absence of DISPLAY STILL turns it off for sure.
    _gui_force_disabled = (
        os.environ.get("YILDIZ_NO_GUI", "").strip().lower() in ("1", "true", "yes", "on")
        or not os.environ.get("DISPLAY")
    )
    _gui_requested = bool(args.gui) or (
        os.environ.get("YILDIZ_GUI", "").strip().lower() in ("1", "true", "yes", "on")
    )
    if _gui_force_disabled or not _gui_requested:
        if _gui_requested and _gui_force_disabled:
            print(
                f"[{redcolor}simple_follow{endcolor}] GUI requested but closed "
                f"(YILDIZ_NO_GUI=1 or no DISPLAY)"
            )
        else:
            print(
                f"[{redcolor}simple_follow{endcolor}] GUI is off (default). "
                f"To turn on --gui / YILDIZ_GUI=1 -- the cycle decreases from 20 Hz to ~5 Hz, "
                f"DO NOT open during flight"
            )
        gui = None
    else:
        print(
            f"[{redcolor}simple_follow{endcolor}] Starting guidance GUI ... "
            f"(WARNING: drawing PyQt5 in the same process holds IOL, cycle drops to ~5 Hz)"
        )
    ...
    while running:
        ...
        # MEASURED loop step: base on half nominal step (against timer bounce running twice in one cycle) and
        # capped by 0.5 s (lest a long hang throw out all the integrators).
        ...
        if slow_loop_streak >= 10 and loop_start - last_slow_loop_warn > 5.0:
            ...
            print(
                f"[{redcolor}simple_follow{endcolor}] *** LOOP SLOW: "
                f"measured {1.0 / max(dt_avg, 1e-6):.2f} Hz (desired "
                f"{args.loop_hz:.1f} Hz, dt {dt_avg * 1000.0:.0f} ms) -- "
                f"{slow_loop_streak} sequential loop. Reduce CPU load "
                f"(YILDIZ_NO_GUI=1 / video recording off) ***"
            )
            last_slow_loop_warn = loop_start

        # Command authority 10 is polled at Hz; connection error preserves the last known value (so that the
        # short Redis outage does not turn into a mod war on the vehicle).
        if authority_client is not None and loop_start - last_authority_poll >= 0.1:
            last_authority_poll = loop_start
            try:
                _v = authority_client.get("command_authority")
                new_authorized = (_v is None
                                or _v.decode("utf-8", "replace") != "visual")
            except Exception:
                new_authorized = position_based_authorized
            if new_authorized != position_based_authorized:
                position_based_authorized = new_authorized
                print(
                    f"[{redcolor}simple_follow{endcolor}] COMMAND AUTHORITY: "
                    + ("positioned (back handoff, guidance maintained)"
                       if position_based_authorized else
                       "display (setpoint/mode commands stopped, "
                       "estimator continues to work)")
                )
        ...
        # Sim-time rate: how far has the hunter's own clock (TIME_BOOT_MS) advanced in the wall clock window
        # of ~5 s? Falling below 1.0 tells you that SITL is lagging behind real time -- this run is not valid
        # no matter what the drive does.
            ...
            if simtime_ref_wall is None:
                ...
                if (
                    ...
                    print(
                        ...
                        f"BEHIND: sim/wall ratio {simtime_ratio:.2f} (< 0.90 ) -- "
                        f"SITL can't keep up with real time, results of this run "
                        f"is invalid ***"
                    )
        ...
        if target_age > args.target_timeout:
            ...
            ) and position_based_authorized and failsafe.mode and failsafe.mode != commanded_mode:
                try:
        ...
        # --- Mission health verdict for this loop --- Everything the setpoint depends on must be trustworthy.
        # The estimator state is checked explicitly: once a filter goes non-finite it never recovers on its
        # own, and every downstream setpoint would be NaN.
        ...
        send_setpoint = (
            bool(directive["send"]) and not failsafe.tripped and position_based_authorized
        )
        if failsafe.tripped and position_based_authorized:
        ...
        if send_setpoint:
            ...
            #
            # 2026-08-02: old budget was d_allow = carrot_max + max(0, d_xy - carrot_range); Because 1:1 was
            # growing, 821 was "clamped" to 751 m, even at an offset of m, and NO limits were being applied
            # despite the appearance of carrot_limited=1. Added a FIXED METRIC budget, valid ALWAYS: the command
            # point can be at most speed_cmd * BUDGET_S + BUDGET_MARGIN_M ahead of the fighter. The position error
            # thus fails to inject an extra velocity of ~20 m/s into the internal P-term of ArduCopter (dataflash:
            # desired velocity 39.4 m/s = 2 x WPNAV_SPEED).
            if not args.position_only:
                ...
                # FIXED METRIC CEILING: not conditional, valid every cycle.
                cmd_speed_xy = float(
            ...
            # Acceleration feedforward: midcourse lead aid, CHASE only. Faded to zero as the pursuer enters the
            # terminal-extension band (adding target acceleration there causes blow-through overshoot) and
            # suppressed once the terminal latch has frozen the aim.
            if (
            ...
            if authority_client is not None:
                # handoff seed: seeds display code integrators with this last command. However, the command is
                # refreshed in sent cycles; the consumer distinguishes staleness by t_mono.
                try:
                    authority_client.set("handoff_state", json.dumps({
                        "t_mono": loop_start,
                        "cmd_vel_ned": [float(slot_vel[0]), float(slot_vel[1]),
                                        float(slot_vel[2])],
                        "cmd_yaw_rad": float(last_yaw_cmd),
                        # MEASURED BODY YAW SPEED (2026-08-04): the display controller has to subtract our OWN spin from the
                        # disturbance residual (d_ex = measured angle velocity - predicted by our own motion). At the moment
                        # of handoff, the vehicle was generally TURNING, and since the other party learned the yaw speed one
                        # cycle late, some of our own rotation was entering the jammer as a fake "target movement". This is
                        # our OWN situation; outside target telemetry constraint.
