...
def baseline_mask(cam1: CamStream, cam2: CamStream,
    ...
    if t2.size == 0:
        return keep
    # station config work piecewise-constant, so evaluate before per external combo
...
def build_windows(root: str = DEFAULT_ROOT, sessions: Sequence[str] = SESSIONS,
    ...
    for sess in sessions:
        ...
        for t0, t1 in segment_bounds(cam1.t, min_rows=min_rows):
            ...
            windows.append(w)
            if verbose:
                print(f"   {w.label} stop= {w.duration:5.0f} s "
...
if __name__ == "__main__":
    ...
    print("=" * 78)
    wins = [w for w in build_windows(args.root) if w.duration >= args.min_stop]

    for w in wins:
        geo = rig_geometry(w)
        print()
        print("=" * 78)
        print(f"WINDOW {w.label} stop= {w.duration:.0f} s  "
