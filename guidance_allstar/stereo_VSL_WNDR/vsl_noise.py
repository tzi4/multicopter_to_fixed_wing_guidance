...
2. **Whiteness** -- the autocorrelation of the residual sequence. This is the property that R tuning
usually gets wrong. A Kalman filter assumes successive measurements are independent; if they are
correlated, feeding them at full rate makes the filter believe it has N independent looks when it
has far fewer, and it becomes overconfident no matter how carefully sigma itself was measured. VSL
already documented this in selfcal_ekf.py ("Innovations are not white; lag-1 autocorrelation is
approximately +0.92 at 9-10 Hz") and handle it by thinning to 3 Hz.
