...
Implementation notes: * Sequential scalar updates. Each of the four angles is applied one at a time,
recomputing h(x) and its jacobian against the partially-updated state. That is the exact chain-rule
factorisation of the joint likelihood p(z1..z4) = p(z1) p(z2|z1) p(z3|z1,z2) p(z4|...), so the mode
likelihoods handed to the IMM stay correct, and it needs no matrix inversion. * Joseph-form
covariance update, matching filterwndr's linear modes. * Gating is decided BEFORE from the IMM's
combined prior and then applied identically to every mode. If each mode gated independently they
would be conditioning on different measurement sets and their likelihoods would no longer be
comparable -- which silently corrupts the mode probabilities. * The caller must PREDICT before
calling here. filterpy's IMM computes cbar during mixing, and mu = cbar * likelihood; updating
without a preceding predict consumes a stale cbar. Same invariant the position pipeline relies on
(see the OOSM warning in filterwndr.py).
"""
...
# ------------------------------------------------------------------ Per-frame measurement plan
# (gating decided before, on the IMM prior)
# ------------------------------------------------------------------
class ScalarSpec:
