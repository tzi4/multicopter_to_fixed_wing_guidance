...
def main():
    ...
    while running:
        ...
        if recovery.state == MissRecoveryController.CHASE:
            ...
            # Once the aim is FROZEN, drop the velocity feedforward with it. A fixed position setpoint plus ~ 20
            # m/s of target-velocity FF is a contradiction, and the moment the pursuer overshoots that point the
            # position error reverses while the FF still pushes forward -> the controller fights itself, pitches
            # back at high thrust and balloons (log 110805 ). Zeroing it makes the latched phase a clean "fly
            # through to this point and stop", which is what the extension already intends.
