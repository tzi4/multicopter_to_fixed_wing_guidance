...
** 2 . What R does this camera want?** A scale sweep on the measured sigma, and -- more importantly
-- a feed-rate sweep. The residuals are strongly autocorrelated (see vsl_noise .py), so a filter fed
at full rate with white-noise R believes it has far more independent information than it does.
Thinning and inflating R are two ways to pay the same debt; the sweep says which the filter prefers.

**3. Rays or triangulate-first?** Two architectures over identical data:

  `` rays `` the IMM updates directly on the four angles (current design). The measurement is what
  the sensor actually reports, the anisotropic depth uncertainty falls out of the geometry, and a
  single camera still constrains the track.  `` tri `` per-frame ML triangulation to a 3D point, fed
  to the IMM as a position measurement with the CRLB covariance from `` triangulation_covariance ``
  . Simpler reuses the position pipeline that already flies -- but it needs both cameras every frame
  and it linearises the geometry once per frame rather than inside the filter.
...
def site_windows(args) -> Tuple[List[Window], Tuple[CamCalibration, CamCalibration], EnuFrame, Dict]:
    ...
    wins = [w for w in build_windows(verbose=False) if w.duration >= args.min_stop]
...
def main() -> None:
    ...
    ap.add_argument("--min-stop", type=float, default=300.0)
