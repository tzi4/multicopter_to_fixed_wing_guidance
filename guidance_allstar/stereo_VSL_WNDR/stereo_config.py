...
#
# It does not pay for itself even where it is safe: on the synthetic bench it is marginally WORSE than
# off at both 6 m and 100 m baselines ( 5.86 vs 5.47 m and 0.43 vs 0.41 m). The CT modes get their
# turn rate from the IMM's own mixing perfectly well; the hint was insurance that turned out to cost
# more than it paid. Enable it only for a fix stream whose noise is small compared with the target's
# displacement between hints.
...
#
# A constant ANGULAR offset and an error in the camera POSITION are not distinguishable from a single
# deployment's residuals, and the fitter will happily pour one into the other. Fitting with the
# (wrong) entered CAM2 position gave CAM1 yaw -2.08 deg; fitting with the position free gave -0.45
# deg . So ~ 1.6 deg of that "boresight" was really an 8 m survey error wearing an angular costume. It
# cancels at the range it was fitted at and drifts elsewhere: error by range band was 4.4 / 5.8 / 11.2
# m for the bias-only fit versus 4.9 / 4.9 / 7.3 m once the position was corrected too.
#
...
#
# A persistent, direction-consistent skew is extrinsic misalignment, not noise. This tracks a slow
# per-camera yaw/pitch offset and can feed it back as a correction. It refines whatever was entered
# above rather than replacing it. Off by default: verify against a known-good rig first.
