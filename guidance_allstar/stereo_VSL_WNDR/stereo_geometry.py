...
class Camera:
    ...
    def __init__(
        ...
        # Boresight correction SUBTRACTED from incoming measurements. Applied once at ingest
        # (StereoTracker.process) so that triangulation, the skew gate, the init seed and the filter update
        # all see the same corrected angles -- before 2026-07-31 only the update did, which meant a
        # hand-entered offset silently left the geometry path biased.
