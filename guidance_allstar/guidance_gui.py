...
class _ParamStore:
    ...
    def _reload(self):
        try:
            mode_value = importlib.import_module(self.module_name)
            importlib.reload(mode_value)
        except Exception as exc:
            print(f"[gui] WARNING: could not reload {self.module_name}: {exc}")
            return
        with self._lock:
            self._params = {}
            for name in dir(mode_value):
                if name.startswith("_"):
                    continue
                val = getattr(mode_value, name)
                if callable(val) or isinstance(val, type):
                    continue
                self._params[name] = val
            try:
                path = mode_value.__file__
                if path:
    ...
    def set_param(self, name, value_str):
        ...
        path = getattr(mode_value, "__file__", None)
...
class _ParamDocs:
    ...
    def _config_path(self):
        try:
            mode_value = importlib.import_module(self.module_name)
            return getattr(mode_value, "__file__", None)
        except Exception:
            return None
...
class GuidanceGUI:
    ...
    def _run_qt(self):
        ...
        self._win.setWindowTitle("Guidance · Estimator Console")
    ...
    def _rebuild_param_grid_qt(self):
        ...
        for i, (name, val) in enumerate(sorted(params.items())):
            ...
            if not editable:
                entry.setToolType("read-only (non-scalar parameter)")
    ...
    def _run_tk(self):
        self._root = tk.Tk()
        self._root.title("Guidance · Estimator Console")
        ...
        tk.Label(
            header, text="  ● Guidance Estimator Console", bg=PAL["surface"],
