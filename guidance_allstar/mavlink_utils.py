...
# To translate HEARTBEAT's number custom_mode into the human-readable mode name (ArduPilot Copter: 4 =
# GUIDED, 5 = LOITER, ...). If pymavlink is not present (pure unit testing) we silently drop to the
# raw number -- this module should be able to be imported without MAVLink.
try:
...
class MavStateReader(threading.Thread):
        ...
        while True:
            ...
            if msg is not None:
                ...
                if mtype == self.msg_types[0]:
                    ...
                    # AUTOPILOT MODE + LINK HEALTH (2026-08-07, actual flight logging). It answers two questions: (a)
                    # "What mode was the vehicle in at the time of the accident?" -- A vehicle that has fallen out of
                    # GUIDED (LAND/RTL/STABILIZED) is NOT listening to our setpoints ANYWAY; no guidance column indicates
                    # this. (b) “is the telemetry still flowing?” -- If the heartbeat age increases, everything we read is
                    # RETURNED data (get() silently flies with the old value since it returns from the cache; 2026-07-24
                    # review's fear). Only the autopilot's own component (AUTOPILOT1) is accepted so that gimbal/ground
                    # station heartbeats are not counted.
                    if getattr(msg, "autopilot", 0) == 0:      # INVALID = GCS
                        continue
                    try:
                        label_item = (_mavutil.mode_string_v10(msg)
                    ...
                    with self.lock:
                        if "hb" not in self._extras:
                            print(f"[mav_reader] First HEARTBEAT received: "
                                  f"mod={label_item or msg.custom_mode}")
                        self._extras["hb"] = (label_item, int(msg.custom_mode),
                                              int(msg.base_mode))
    ...
    def get_heartbeat(self):
        """Last HEARTBEAT: (mode_name, custom_mode, age_s_value) or None.

        DIFFERENCE FROM OTHERS: THERE IS NO STAGE CONTROL, because what is sought is the staleness
        ITSELF. If age_s_value is growing, the connection with the autopilot has been lost and all
        pos/vel/att values ​​read at that moment have returned. Only for LOG; The control path does
